...
public class DogEnemyChasing : EnemyState {
    ...
    {
        if (m_SoundDogEnemy == null) enemy.GetComponent<SoundDogEnemy>();

        if (m_SoundDogEnemy.CanSeePlayerAndRobot())
        {
            //ターゲットの場所へ向かう
            m_SoundDogEnemy.m_Agent.destination = m_SoundDogEnemy.CheckPlayerAndRobot().transform.position;
            //ターゲットとの距離
            float distance = Vector3.Distance(m_SoundDogEnemy.GetEnemyPosition(), m_SoundDogEnemy.CheckPlayerAndRobot().transform.position);
